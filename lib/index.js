...
class Resolver {
  ...
  generateFileId(storage_index) {
    return ethers.utils.formatBytes32String(storage_index);
  }
  ...
  async revokeSigned(file_id, user, accessTypes, accessKey, sig) {
    return await this.contract.updateACKSigned(file_id, user, accessTypes, accessKey, 0, sig.address, sig.v, sig.r, sig.s)
  }

  async revokeRawTransaction(file_id, user, accessTypes, accessKey) {
    return await this.sign(["bytes32", "address", "bytes32", "bytes32", "uint256"], [file_id, user, accessTypes, accessKey, 0], this.wallet.privateKey);
  }
